...
async function processJob(job: Job, dir: string): Promise<void> { job.status = 'processing'; try { let mediaCount = 0; for (const message of job.messages) for (const media of message.media ?? []) if (media.data) { const ext = media.type === 'audio' ? 'webm' : media.type === 'video' ? 'mp4' : 'bin'; await fs.writeFile(path.join(dir, `media-${++mediaCount}.${ext}`), Buffer.from(media.data, 'base64')); delete media.data; } await fs.writeFile(path.join(dir, 'messages.json'), JSON.stringify(job.messages, null, 2)); const prompt = `Read ${path.join(dir, 'messages.json')} and inspect any media files in ${dir}. Triage these WhatsApp support messages into one GitHub issue for ${job.repo}. Return ONLY JSON with title, body, labels. Infer affected school/product/module only when supported. Include reproduction, expected/actual behavior, evidence references, and open questions.`; const raw = await command('gemini', ['-p', prompt]); const parsed = JSON.parse(raw.match(/\{[\s\S]*\}/)?.[0] ?? ''); job.issue = parsed; const created = await command('gh', ['issue', 'create', '--repo', job.repo, '--title', parsed.title, '--body', parsed.body]); job.issueUrl = created.trim().split(/\s+/).find(value => value.includes('/issues/')) ?? created.trim(); job.status = 'completed'; job.completedAt = new Date().toISOString(); } catch (error) { job.status = 'failed'; job.error = error instanceof Error ? error.message : String(error); } const items = await history(); items.unshift(job); await saveHistory(items); }
